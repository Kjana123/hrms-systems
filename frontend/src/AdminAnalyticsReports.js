...
const AdminAnalyticsReports = ({ showMessage, apiBaseUrl, accessToken, authAxios, adminStats, handleAnalyticsClick, darkMode }) => {
    ...
    if (loading) {
        return (
            <div className={`flex items-center justify-center p-8 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                <div className="w-4 h-4 rounded-full animate-pulse bg-blue-600 mr-1"></div>
                <div className="w-4 h-4 rounded-full animate-pulse bg-blue-600 mr-1"></div>
    ...
    if (!stats) {
        return (
            <section className={`p-6 rounded-lg shadow-md transition-colors duration-300 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
    ...
    return (
        <div className={`p-6 rounded-lg shadow-md transition-colors duration-300 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
            <h2 className="text-2xl font-semibold mb-6">Analytics & Reports</h2>
            ...
            <section className="mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className={`p-6 rounded-lg shadow-md flex items-center justify-between ${darkMode ? 'bg-blue-900' : 'bg-blue-100'}`}>
                        <div>
                    ...
                    {/* Total Working Hours Card - NEWLY ADDED */}
                    <div className={`p-6 rounded-lg shadow-md flex items-center justify-between ${darkMode ? 'bg-green-900' : 'bg-green-100'}`}>
                        <div>
                            <h3 className="text-lg font-semibold text-green-800 dark:text-green-200">Total Working Hours (This Month)</h3>
                            <p className="text-4xl font-bold text-green-600 dark:text-green-400">
                                {formatDuration(stats.grand_total_working_hours)}
                            </p>
                        </div>
                    ...
                    >
                        <div>
                            <h3 className="text-lg font-semibold text-blue-800 dark:text-blue-200">Total Present Today</h3>
                            <p className="text-4xl font-bold text-blue-600 dark:text-blue-400">{stats.presentToday || 0}</p>
                        </div>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-blue-500 dark:text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    >
                        <div>
                            <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-200">Pending Leaves</h3>
                            <p className="text-4xl font-bold text-yellow-600 dark:text-yellow-400">{stats.pending_leave_requests || 0}</p>
                        </div>
                    ...
                    >
                        <div>
                            <h3 className="text-lg font-semibold text-indigo-800 dark:text-indigo-200">Pending Corrections</h3>
                            <p className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">{stats.pending_correction_requests || 0}</p>
                        </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Total Working Hours</th> {/* NEW COLUMN */}
                            </tr>
                        </thead>
                                ...
                                monthlySummary.map(summary => (
                                    <tr key={summary.user_id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                            {formatDuration(summary.total_working_hours)} {/* Display total working hours for the month */}
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    <td colSpan="9" className="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">No monthly summary data found for the selected period.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
