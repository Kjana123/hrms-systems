...
const Message = ({ message, type, clearMessage }) => {
    ...
    return (
        <div className={`fixed top-4 right-4 p-4 rounded-lg shadow-lg flex items-center space-x-3 z-50 ${bgColor} ${textColor}`}>
            {type === 'success' ? (
            ...
            {clearMessage && (
                <button onClick={clearMessage} className="ml-auto p-1 rounded-full hover:bg-white hover:bg-opacity-20 transition-colors duration-150">
